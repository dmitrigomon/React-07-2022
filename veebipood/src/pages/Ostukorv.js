...
function Ostukorv() {
  ...
  // tuhjenda funktsiooni
  // mitu toodet on ostukorvis
  // dunaamilist valjakuvamist
 ...
 return ( 
    <div>
     ...
     <div>{ostukorv.map((element, index) => 
        <div key={index}>
          {element.nimi}  - {element.hind} eur
          <button onClick={() => kustuta(index)}>X</button>
          <button onClick={() => lisa(element)}>+</button> 
        </div>)}
     </div>
     { ostukorv.length > 0 && <div>Kokku {arvutaKogusumma()} eur</div>}
    </div>
   );
}
